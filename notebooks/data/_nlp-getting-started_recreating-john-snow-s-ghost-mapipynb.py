...
# Reading in the data
deaths = pd.read_csv("../input/john-snow-ghost-map/Recreating John Snow's Ghost Map/datasets/deaths.csv")
...
# Import the data
pumps = pd.read_csv("../input/john-snow-ghost-map/Recreating John Snow's Ghost Map/datasets/pumps.csv")

# Create `locations_pumps` by subsetting only Latitude and Longitude from the dataset 
locations_pumps = pumps[['X coordinate', 'Y coordinate']]

# Create `pumps_list` by transforming the DataFrame to list of lists 
...
dates = pd.read_csv("../input/john-snow-ghost-map/Recreating John Snow's Ghost Map/datasets/dates.csv", parse_dates=['date'])
...
#dates['day_name'] = dates.date.dt.weekday_name  (PLEASE HELP ME IN THIS LINE, If you check this kernel)
...
# The data John Snow collected points to that:
