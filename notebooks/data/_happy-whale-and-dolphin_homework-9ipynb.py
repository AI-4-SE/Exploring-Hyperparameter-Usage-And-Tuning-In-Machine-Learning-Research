...
end = start.shift(years=4)
end = end.shift(days=-1)
print(f"start_date: {start.date()} end_date: {end.date()}")
print("=======================================")
sumdata,meandata = [],[] #宣告空白列表
Q1,Q2,Q3,Q4 = 0,0,0,0 #宣告空白資料放置處
for m in arrow.Arrow.interval('month',start,end,3): #將時間切成3個月一季並加總資料放入sumdata列表中
    start_date = str(m[0].date())
    end_date = str(m[1].date())
    print(f"{start_date},{end_date}")
    sumdf = df[df['日期'].between(start_date,end_date)].sum()
    sumdata.append(sumdf)
print("=======================================")
for i in range(0,16,4): #將不同年的同一季的資料加總在一起，區分成四大季
    Q1 += sumdata[i]
    Q2 += sumdata[i+1]
    Q3 += sumdata[i+2]
    Q4 += sumdata[i+3]
#將四大季資料放入meandata列表中，並平均到小數點後第二位
meandata.append("===========2018-2021 Q1-mean===========")
meandata.append(round(Q1/4,2))
meandata.append("===========2018-2021 Q2-mean===========")
meandata.append(round(Q2/4,2))
meandata.append("===========2018-2021 Q3-mean===========")
meandata.append(round(Q3/4,2))
meandata.append("===========2018-2021 Q4-mean===========")
meandata.append(round(Q4/4,2))
for j in meandata: #輸出meandata列表
    print(j)
