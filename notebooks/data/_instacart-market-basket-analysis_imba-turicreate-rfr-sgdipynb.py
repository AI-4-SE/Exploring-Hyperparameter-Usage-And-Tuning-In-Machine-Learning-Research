...
ou = orders[orders.eval_set != 'test']
...
#set test dataset
test = orders[orders.eval_set=='test'].drop(['eval_set','order_id'],axis=1)
...
# # Convert to turicreate dataframe
...
#training
recommender =  turicreate.recommender.ranking_factorization_recommender.create(train_data, user_id='user_id', item_id='product_id', target='target', solver='sgd', user_data=None, item_data=content_data, verbose=True)
...
thresholds = {
    'mean': result.score.mean(),
    'std' : result.score.std(),
    'zero' : 0,
    'q05' : result.score.quantile(q=0.05),
    'q10' : result.score.quantile(q=0.1),
    'q15' : result.score.quantile(q=0.15),
    'q25' : result.score.quantile(q=0.25),
    'q50' : result.score.quantile(q=0.5)
    }
...
for key,item in thresholds.items():
    results[key].to_csv('submission_turi_rfr_sgd_'+str(key)+'.csv',index=False)
